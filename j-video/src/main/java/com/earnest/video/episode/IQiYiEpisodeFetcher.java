...
import com.earnest.video.exception.EpisodeExtractException;
...
public class IQiYiEpisodeFetcher extends HttpProxyPoolSettingSupport implements EpisodeFetcher {
    ...
    private static final Pattern EPISODE_EXTRACT_PATTERN = Pattern.compile("\"vlist\":(\\[\\{.+\\}\\])");
    ...
    private static List<Episode> extractJsonString(String entityString) throws IOException {
        Matcher matcher = EPISODE_EXTRACT_PATTERN.matcher(entityString);
    ...
    private static Function<JSONObject, Episode> mapToEpisodeEntity() {
        return e -> {
            ...
            episode.setDuration(e.getIntValue("timeLength"));
            episode.setRawValue(e.getString("vurl"));
            ...
            episode.setProperties(Map.of("tvid", e.getString("tvQipuId"), "vid", e.getString("vid")));
